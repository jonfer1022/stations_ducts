...
class disasterRiskController {
  ...
  static async getTablePercentageDisasterRiskCont({ 
    ducts_id,
    sector,
    segment
  } : ParsedQs): Promise<object> {
    return new Promise( async (resolve, reject) => {
      try {
        const res = await disasterRiskModel.getTablePercentageDisasterRiskDB(
          ducts_id?.toString(),
          sector?.toString(),
          segment?.toString()
        );
        ...
        console.error("An error ocurred getTablePercentageDisasterRiskCont: ",error);
        reject(error);
      }
    })
  }
